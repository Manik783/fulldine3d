...
const adddish = () => {
  ...
  const handleGlbUpload = async () => {
    ...
    const formGlb = new FormData();
    formGlb.append("file", formData.glbFile);

    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/dish/upload_glb`,
        formGlb,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      setFormData({ ...formData, glb_url: response.data.fileUrl });
      alert("GLB file uploaded successfully");
    } catch (error) {
  ...
  const handleUsdzUpload = async () => {
    if (!formData.usdzFile) {
      alert("Please select a .usdz file to upload");
      return;
    }

    const formUsdz = new FormData();
    formUsdz.append("file", formData.usdzFile);

    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL}/dish/upload_usdz`,
      ...
      setFormData({ ...formData, usdz_url: response.data.fileUrl });
      alert("USDZ file uploaded successfully");
    } catch (error) {
      console.error("Error uploading USDZ file:", error);
      alert("Failed to upload USDZ file");
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_BASE_URL}/admin/add_dish`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      >
        ...
        <div className="mb-4">
          ...
          <input
            ...
            accept=".usdz"
            onChange={(e) =>
              setFormData({ ...formData, usdzFile: e.target.files[0] })
            }
            className="w-full px-4 py-2 border rounded-md"
          />
          <button
            type="button"
            onClick={handleUsdzUpload}
